...
const getobtainCursoUser = async (req, res) => {
  try {
    const result = await ControllerUserCursos.obtainCursoUser(req);
    res.json(result);
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
...
module.exports = { postCreateUserCurso, getobtainCursoUser };
